...
(function() {
  var coffee, coffeelint, concat, config, cucumber, del, gulp, imagemin, jasmine, jsdoc, notify, runSequence, sass, uglify, uglifycss;
  ...
  gulp.task(config.tasks["default"], function() {});
  ...
  gulp.task(config.tasks.jsdoc, function() {
    return gulp.src("./src/scripts/*.js").pipe(jsdoc('.documentation'));
  });

  gulp.task(config.tasks.build, [config.tasks.copyToBuildFolder, config.tasks.sassCompressedCopyToBuild, config.tasks.buildScripts], function() {});

  gulp.task(config.tasks.clean, function(callback) {
    return del([config.build.folder], callback);
  });

  gulp.task(config.tasks.copyToBuildFolder, function() {
  ...
  gulp.task(config.tasks.sassCompressedCopyToBuild, function() {
  ...
  gulp.task(config.tasks.buildScripts, function() {
    return gulp.src(config.src.main).pipe(coffee()).pipe(uglify()).pipe(gulp.dest(config.build.scripts));
  });
  ...
  gulp.task(config.tasks.copyToSourceVendor, function() {
